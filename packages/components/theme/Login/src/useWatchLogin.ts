...
export const useWatchLogin = () => {
   ...
   *
   * 有 3 个页面级别认证：
   * 1、单页面级别：先校验单页面的登录信息，如果失败，则校验页面领域级别登录信息，如果失败，则跳转登录页面
   * 2、页面领域级别：一个领域就是一个登录信息组，支持多个文章在一个组里认证
   * 3、共享页面级别：一旦在任意文章登录过，则可以访问【非单页面级别、非页面领域级别】的其他任意文章
   */
  const watchPages = () => {
    if (!isClient) return;
    ...
    watch(
      ...
      newVal => {
        ...
        if (isPage) {
          // 来到此处，代表单页面级别认证和页面领域级别认证失败，则跳转到登录页
