...
```ts
const formatDate: (date: Date | string, format = "yyyy-MM-dd hh:mm:ss") => string;
```
...
```ts
const formatDiffDate: (startDate: Date | string, endDate?: Date | string) => string;
```
...
```ts
const formatDiffDateToDay: (startDate: string, endDate?: string) => number;
```
