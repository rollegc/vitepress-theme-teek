...
const tkConfig = tkThemeConfig({
  banner: {
    ...
    pureBgColor: "#e5e5e5", // Banner 背景色，bgStyle 为 pure 时生效
    imgSrc: ["/img/bg1.jpg", "/img/bg2.png"], // Banner 大图链接。bgStyle 为 partImg 或 fullImg 时生效
    imgInterval: 15000, // 当多张大图时（imgSrc 为数组），设置切换时间，单位：毫秒
    imgShuffle: false, // 大图是否随机切换，为 false 时按顺序切换，bgStyle 为 partImg 或 fullImg 时生效
    imgWaves: true, // 是否开启 Banner 大图波浪纹，bgStyle 为 partImg 或 fullImg 时生效
    mask: true, // Banner 大图遮罩，bgStyle 为 partImg 或 fullImg 时生效
...
interface TkThemeConfig {
  /**
   * 壁纸模式，在首页最顶部进入全屏后开启，仅当 (banner.bgStyle = 'fullImg' && banner.imgSrc 不存在) 或 bodyBgImg.imgSrc 存在才生效
   */
  wallpaper?: {
    /**
     * 是否启用壁纸模式
