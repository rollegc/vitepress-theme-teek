...
在官方配置中通过 `extends` 可以将主题配置合并到官方配置里，也就是说你完全可以在主题配置里添加官方的配置项，但是不能反过来，如：
...
公告组件只提供基础功能，不提供任何内容的渲染，完全需要你自定义组件，然后通过 `notice-content` 插槽传进来。
...
评论配置，目前内置 `giscus`、`twikoo`、`waline`、`artalk` 四种评论插件，如果这四个都不符合你需求，可以自己实现评论插件，然后传入进来。
