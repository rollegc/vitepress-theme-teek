...
permalink: /config/frontmatter
---

`frontmatter` 支持基于页面的配置。在每个 Markdown 文件中，可以使用 `frontmatter` 配置来覆盖 [主题配置](/config/theme) 中的部分选项。

## 首页配置

### features

Teeker 提供了 `features` 选项，用于在首页展示一些功能介绍，且为了不与 Vitepress 的 `features` 冲突，所以 Teeker 提供了 `tk.features` 或者 `tk.banner.features` 来配置首页的 `features`。
...
author:
  name: TianKe
  link: https://github.com/Kele-Bingtang/vitepress-theme-teeker
article:
  showCategory: true
breadcrumb:
  separator: -
...
除了支持覆盖主题配置的选项，Teeker 还提供了以下额外的选项：
