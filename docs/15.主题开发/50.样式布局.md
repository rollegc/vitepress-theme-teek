...
Teeker 并没有和普通的 Vue 项目一样，使用如下模板进行编写组件：
...
Teeker 专门创建 `styles` 目录用于存放组件样式，然后将所有的组件样式汇总到一个 `index.scss`（入口样式文件），最后在 `index.ts`（入口运行文件）分别引入 `styles/index.scss`（入口样式文件）和 `layout/index.vue`（入口组件）。
...
::: tip
这也就是为什么在 `.vitepress/theme/index.ts` 里单独引入 Teeker 样式的原因。
:::

## 样式结构设计

Teeker 的样式结构设计通用遵循单一原则：每个样式文件只负责渲染单独的模块（组件），如：
...
当然，对于不喜欢折腾、研究的小伙伴来说，Teeker 也提供了 `nav.scss` 样式文件，该文件内部没有编写任何样式代码，而是引入了 `nav-xxx.scss` 等样式文件，用于快速引入 `nav` 的所有样式文件。
...
在 `components` 目录下，Teeker 会给每个组件生成对应的样式文件，文件名与组件名相同（首字母小写）。

`index.scss` 文件是入口样式文件，它将导入 `base.scss`、`var` 目录下的样式文件，以及 `components` 目录下所有组件的样式文件。

如果需要按需加载组件，您必须要引入 `base.scss` 文件，这是 Teeker 的核心主题样式文件，然后按需引入 Vue 组件和 `components` 目录下的对应组件的样式文件。

## 命名空间

Teeker 并没有简单的直接给一个 `div` 元素添加 `class="button"`，然后在 CSS 里 `.button {}` 定义样式，Teeker 使用 **命名空间** 的设计思想，给每个组件添加唯一标识，确保不同组件的相同 `class` 发生样式冲突。
...
首先 Teeker 在 `styles/mixins/config.scss` 文件中定义了命名空间变量 `$namespace`：
...
::: tip
将 `$namespace: tk` 改为 `$namespace: xx`（xx 为你的项目名/框架名），那么没人知道它是 teeker，它已经完全属于你。
:::

## 什么是 BEM

Teeker 使用 BEM 规范进行样式编写，并使用 SCSS 进行样式编写。
...
定义一个 Hooks 文件 [useNamespace.ts](https://github.com/Kele-Bingtang/vitepress-theme-teeker/blob/master/vitepress-theme-teeker/src/hooks/useNamespace.ts) 来封装命名空间和 BEM 规范，[命名空间](#命名空间) 和 [BEM 规范](#什么是-bem) 在上文已经介绍过了。
...
定义 SCSS 文件 [bem.scss](https://github.com/Kele-Bingtang/vitepress-theme-teeker/blob/master/vitepress-theme-teeker/src/styles/mixins/bem.scss) 来封装命名空间和 BEM 规范，
...
Teeker 给所有的 CSS Var 变量都添加命名空间，如：

```css
--tk-lower-color-1: #86909c;
```

但是每次定义都加 `--tk-` 都比较麻烦，所以 Teeker 提供了 `set-css-var` Mixin 和 `getCss-Var` 函数来进行封装：
