...
定义一个 Hooks 文件 [useNamespace.ts](https://github.com/Kele-Bingtang/vitepress-theme-teek/tree/master/packages/hooks/useNamespace.ts) 来封装命名空间和 BEM 规范，[命名空间](#命名空间) 和 [BEM 规范](#什么是-bem) 在上文已经介绍过了。
