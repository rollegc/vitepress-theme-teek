...
VitePress 的 `.vitepress/config.ts` 在 `node` 环境运行，因此 `config.ts` 文件引入的第三方依赖必须是 `js` 相关文件，在 `.vitepress/theme/index.ts` 文件则可以引入 `ts`、`vue` 等不需要构建的文件。
:::
...
因此你可以不使用 `outDir` 选项，`outDir` 默认为 `dist`，因此它会自动将 components 目录下的文件复制到 dist 目录下。

### mkdist 构建多个 TS 文件

如果插件需要构建多个 TS 文件，则 unbuild 的配置文件内容如下所示：
