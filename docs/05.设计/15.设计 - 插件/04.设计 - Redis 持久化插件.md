...
Hd Security 已经实现了集成 Redis 的插件，具体使用请看 [插件 - 持久层集成 Redis]。
...
首先自定义类 `HdSecurityRedisRepository` 实现 `HdSecurityRepository` 接口，`HdSecurityRepository` 是 Hd Security 内置的持久层组件，支持开发者拓展，具体介绍请看 [设计 - 持久层组件]。
...
具体只需要在配置文件配置 Redis 的连接信息即可，具体请看 [设计 - 持久层集成 Redis]。
