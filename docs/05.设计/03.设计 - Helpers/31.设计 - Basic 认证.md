---
title: 设计 - Basic 认证 Token
date: 2025-01-18 14:48:36
permalink: /design/basic-auth
---

## 前言

Hd Security 提供了一个简单的认证方式，即 Basic 认证，它会自动从上下文组件的请求头中获取账号，并校验账号是否正确。

因此开发者需要配置一个固定的账号来和前端传的账号进行匹配，在全局配置文件中配置即可。如果没有配置，Hd Security 的默认账号为空字符串。

上下文组件的讲解请看 [设计 - 上下文组件](/design/context-component)。
