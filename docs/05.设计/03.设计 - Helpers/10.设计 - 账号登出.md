...
在 [设计 - 思路设计] 里介绍了实现账号登出的思路，因此本文介绍 Hd Security 实际的账号登出实现。
...
Token 和 loginId 的映射关系，在账号注销是需要清除，在顶人下线、踢人下线则是需要打一个标识，后续通过标识给前端一个有好多的提示。

为了兼容这 3 个登出方式，`exitLoginByToken` 方法提供的第三个参数为 `Runnable` 类型，调用该方法可以传入不同的额外逻辑。
