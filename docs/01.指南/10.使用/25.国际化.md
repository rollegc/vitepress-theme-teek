...
## 全局语言配置 <Badge type="tip" text="v1.0.3" />
...
import Teek, { zhCn, zhTw, en } from "vitepress-theme-teek";
...
const locale = computed(() => {
  ...
  if (lang.value === "zh-TW") return zhTw;
...
比如需要添加中国香港（zh-hk）语言，步骤如下：

1. 创建 `.vitepress/theme/locale/zh-hk.ts` 文件（路径位置任意）
2. 然后参考 Teek 现有的任一 [语言文件](https://github.com/Kele-Bingtang/vitepress-theme-teek/tree/master/vitepress-theme-teek/src/locale/lang)，将里面的内容拷贝到 `zh-hk.ts` 文件中，并将所有内容（Value）修改为 `zh-hk` 语言
