...
在 Teek 中对 `vitepress-plugin-auto-frontmatter` 插件进行了增强，额外实现了一些常用的功能，能够快捷调整 `frontmatter` 的内容。

不仅可以使用规则快速调整 `permalink` 还可以快速调整其他配置项。



使用以下配置对 `frontmatter` 内容进行调整后，如果没有变更，可以选择关闭一些功能以提升启动速度。

例如调整完 `permalink` 可以将 `enablePermalink` 设置为 `false`，这样就会跳过相关的检查。

#### 提示

- 插件默认跳过 `frontmatter.layout` 设置为`home` 或 `false` 的Markdown 文档
-  `permalink` 处理时默认跳过 `frontmatter.catalogue` 设置为`true` 的Markdown 文档 （目录页）
-  开启 `permalink` 功能但未提供 `permalinkRules` 规则时，添加默认规则：`{ folderName: "*", prefix: "/$path/$uuid5" }`
