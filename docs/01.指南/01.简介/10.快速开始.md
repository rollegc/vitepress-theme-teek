...
```sh [pnpm]
pnpm add scss @types/js-yaml @types/markdown-it @types/markdown-it-container -D
pnpm add typescript gray-matter js-yaml markdown-it-container @iconify/vue vitepress-plugin-auto-frontmatter vitepress-plugin-catalogue vitepress-plugin-doc-analysis vitepress-plugin-file-content-loader vitepress-plugin-md-h1 vitepress-plugin-permalink vitepress-plugin-sidebar-resolve
```

```sh [yarn]
yarn add scss @types/js-yaml @types/markdown-it @types/markdown-it-container -D
yarn add typescript gray-matter js-yaml markdown-it-container @iconify/vue vitepress-plugin-auto-frontmatter vitepress-plugin-catalogue vitepress-plugin-doc-analysis vitepress-plugin-file-content-loader vitepress-plugin-md-h1 vitepress-plugin-permalink vitepress-plugin-sidebar-resolve
```
...
::: tip
如果依赖存在缺失或依赖版本与 Teek 冲突，可以查看 `vitepress-theme-teek/package-release.json` 文件，将 `dependencies` 和 `devDependencies` 中的依赖复制到您项目的 `package.json` 文件里。

其中 `devDependencies` 的 `unbuild`、`vitepress` 依赖不需要安装（去掉），`unbuild` 是一个轻量的构建工具，您的项目已经采用 VitePress 进行构建，因此无需安装这些依赖。
:::
